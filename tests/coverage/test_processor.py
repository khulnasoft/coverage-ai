...
class TestCoverageProcessorFactory:
    def test_create_processor_cobertura(self):
        ...
        assert isinstance(
            processor, CoberturaProcessor
        ), "Expected CoberturaProcessor instance"
    ...
    def test_create_processor_jacoco(self):
        ...
        assert isinstance(
            processor, JacocoProcessor
        ), "Expected JacocoProcessor instance"
    ...
    def test_create_processor_unsupported_type(self):
        with pytest.raises(
            ValueError, match="Invalid coverage type specified: unsupported_type"
        ):
            CoverageProcessorFactory.create_processor(
                "unsupported_type", "fake_path", "app.py"
            )
...
class TestCoverageProcessor:
    ...
    def test_process_valid_coverage_data(self, mocker):
        ...
        time_of_test = 123456
        coverage_data = {
            "file1.py": CoverageData(
                covered_lines=[], covered=80, missed_lines=[], missed=20, coverage=0.8
            ),
            "file2.py": CoverageData(
                covered_lines=[], covered=60, missed_lines=[], missed=40, coverage=0.6
            ),
        }
        ...
        mocker.patch.object(
            processor, "parse_coverage_report", return_value=coverage_data
        )
        ...
        processor._is_coverage_valid.assert_called_once_with(
            time_of_test_command=time_of_test
        )
    ...
    def test_process_zero_lines_coverage(self, mocker):
        ...
        coverage_data = {
            "file1.py": CoverageData(
                covered_lines=[], covered=0, missed_lines=[], missed=0, coverage=0.0
            ),
            "file2.py": CoverageData(
                covered_lines=[], covered=0, missed_lines=[], missed=0, coverage=0.0
            ),
...
class TestCoberturaProcessor:
    ...
    def processor(self):
        ...
        return CoverageProcessorFactory.create_processor(
            "cobertura", "fake_path", "app.py"
        )
...
class TestJacocoProcessor:
    ...
    def test_parse_xml_coverage_report_success(self, mocker):
        ...
        mock_file = mocker.mock_open(
            read_data="package com.example;\npublic class MyClass {"
        )
        mocker.patch("builtins.open", mock_file)
        mocker.patch(
            "xml.etree.ElementTree.parse",
            return_value=ET.ElementTree(ET.fromstring(xml_content)),
        )
