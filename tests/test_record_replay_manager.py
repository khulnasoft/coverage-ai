...
from coverage_ai.record_replay_manager import RecordReplayManager
...
class TestFileHandling:
    ...
    @staticmethod
    def test_get_response_file_path_handle_source_path_with_no_parent_directory(
        tmp_path,
    ):
        """
    ...
    def test_get_response_file_path_with_valid_nested_source(tmp_path):
        ...
        result = manager._get_response_file_path(
            "nested/folder/source_file.py", "tests/test_file.py"
        )
    ...
    def test_get_response_file_path_create_response_directory_if_not_exists(tmp_path):
        ...
        result = manager._get_response_file_path(
            "new_folder/source_file.py", "tests/test_file.py"
        )
...
class TestResponseHandling:
    ...
    @staticmethod
    def test_has_response_file_response_file_exists_returns_true_when_file_present(
        tmp_path,
    ):
        """
    ...
    @staticmethod
    def test_has_response_file_response_file_exists_returns_false_when_file_missing(
        tmp_path,
    ):
        """
    ...
    @staticmethod
    def test_has_response_file_response_file_exists_raises_error_on_empty_source_path(
        tmp_path,
    ):
        """
    ...
    @staticmethod
    def test_has_response_file_response_file_exists_raises_error_on_empty_test_path(
        tmp_path,
    ):
        """
    ...
    def test_load_recorded_response_with_fuzzy_lookup_on_dict_prompts(tmp_path):
        ...
        manager = RecordReplayManager(record_mode=False, base_dir=str(tmp_path))
        manager._calculate_files_hash = Mock(return_value="hash123")
        manager._find_closest_prompt_match = Mock(
            side_effect=lambda prompt, prompts, **kwargs: next(iter(prompts))
        )
        ...
        recorded_prompt = {
            "user": "Generate a function to find all prime numbers up to 100"
        }
        ...
        current_prompt = {
            "user": "Create a function that finds prime numbers below 100"
        }
        result = manager.load_recorded_response(
            "source.py",
            "test.py",
    ...
    def test_load_recorded_response_with_fuzzy_lookup_multiple_prompts(tmp_path):
        ...
        current_prompt = {
            "user": "Create a function for finding prime numbers below 100"
        }
        result = manager.load_recorded_response(
            "source.py",
            "test.py",
    ...
    def test_record_response(test_case, tmp_path):
        ...
        """
        manager = RecordReplayManager(
            record_mode=test_case["record_mode"], base_dir=str(tmp_path)
        )
