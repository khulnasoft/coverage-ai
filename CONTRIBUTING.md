...
This document outlines the guidelines and best practices for contributing to the `coverage-ai` repository.
...
If you're new, we encourage you to take a look at issues tagged with "[good first issue](https://github.com/khulnasoft/coverage-ai/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)".
...
1. Create a personal fork of the `coverage-ai` project using the GitHub UI.

2. Execute these steps with the forked code:

```bash
# Clone your fork to your local file system
git clone git@github.com:<your_github_login>/coverage-ai.git

# Add the original `coverage-ai` repository as the upstream remote
git remote add upstream git@github.com:khulnasoft/coverage-ai.git
