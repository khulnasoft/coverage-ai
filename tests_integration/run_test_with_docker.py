...
def run_test(test_args: argparse.Namespace) -> None:
    ...
    try:
        ...
        image_tag = get_docker_image(
            client, test_args.dockerfile, test_args.docker_image
        )
        ...
        execute_test_in_container(
            container, container_config["command"], container_config["exec_env"]
        )
...
def prepare_container_config(
    test_args: argparse.Namespace, image_tag: str, client: docker.DockerClient
) -> dict:
    """
...
def prepare_record_mode_volume(
    test_args: argparse.Namespace, image_tag: str, client: docker.DockerClient
) -> dict:
    """
    ...
    host_folder_path = (
        f"{Path(__file__).resolve().parents[1]}/{SETTINGS.responses_folder}"
    )
...
def execute_test_in_container(container, command: list, exec_env: dict) -> None:
    ...
    copy_file_to_docker_container(
        container,
        SETTINGS.coverage_ai_host_folder,
        SETTINGS.coverage_ai_container_folder,
    )
...
def validate_test_args(test_args: argparse.Namespace) -> None:
    ...
    """
    if (
        not test_args.source_file_path
        or not test_args.test_file_path
        or not test_args.test_command
    ):
