...
docker build -t coverage-ai-image -f tests_integration/python_readyapi/Dockerfile .

# Start the container in detached mode with the environment variable
echo "Starting the container..."
CONTAINER_ID=$(docker run -d -e OPENAI_API_KEY=$OPENAI_API_KEY coverage-ai-image tail -f /dev/null)
...
docker exec -e OPENAI_API_KEY=$OPENAI_API_KEY $CONTAINER_ID /usr/local/bin/coverage-ai \
