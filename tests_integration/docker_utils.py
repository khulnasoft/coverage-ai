...
def get_docker_image(
    ...
    """
    logger.info(
        f"Starting to get the Docker image {docker_image} with platform {platform}..."
    )
...
def pull_and_tag_docker_image(
    client: docker.DockerClient, docker_image: str, image_tag: str
) -> None:
    """
    ...
    try:
        ...
        logger.error(
            f"Pulled Docker image {docker_image} could not be found for tagging."
        )
...
def copy_file_to_docker_container(
    container: Container, src_path: str, dest_path: str
) -> None:
    """
    ...
    """
    logger.info(
        f"Copying file from {src_path} to {dest_path} in the Docker container {container.name}..."
    )
    ...
    tar_stream.seek(0)
    logger.info(
        f"Sending tar archive to the Docker container {container.name} at {os.path.dirname(dest_path)}..."
    )
    container.put_archive(path=os.path.dirname(dest_path), data=tar_stream)
    logger.info(
        f"File {src_path} successfully copied to {dest_path} in the Docker container {container.name}."
    )


def run_command_in_docker_container(
    container: Container, command: list[str], exec_env: dict[str, Any]
) -> None:
    """
    ...
    try:
        ...
        logger.debug(
            f"The command {joined_command} finished with exit code: {exit_code}"
        )
        if exit_code != 0:
        ...
        logger.error(
            f"Failed to execute the command in the Docker container {container.name}: {e}"
        )
...
def clean_up_docker_container(container: Container) -> None:
    ...
    logger.info(
        f"Stop the Docker container {truncate_hash(container.id, HASH_DISPLAY_LENGTH)}."
    )
    container.stop()

    logger.info(
        f"Remove the Docker container {truncate_hash(container.id, HASH_DISPLAY_LENGTH)}."
    )
    container.remove()
...
def show_progress(
    line: dict, progress: Progress, id_to_task: dict[str, int] | None = None
) -> None:
    """
    ...
    if task_id is None:
        ...
        task_id = progress.add_task(
            description=normalized_status.value,
            total=100,
            completed=0,
            visible=True,
            **task_fields,
