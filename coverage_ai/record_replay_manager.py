...
from coverage_ai.custom_logger import CustomLogger
...
class RecordReplayManager:
    ...
    def load_recorded_response(
        ...
        try:
            ...
            self.logger.info(
                f"Do a direct hash lookup for prompt hash {prompt_hash} under caller {caller}..."
            )

            # Look for the prompt hash in the caller's records
            if prompt_hash in cached_data[caller_name]:
                self.logger.info(
                    f"Record hit for caller {caller_name}() with prompt hash {prompt_hash}."
                )
            ...
            if fuzzy_lookup:
                self.logger.info(
                    f"Trying fuzzy lookup for prompt hash {prompt_hash} under caller {caller}..."
                )
                prompts = {
                    k: v["prompt"]["user"] for k, v in cached_data[caller_name].items()
                }
                ...
                if fuzzy_prompt_hash:
                    self.logger.info(
                        f"Found fuzzy match for prompt hash {fuzzy_prompt_hash} under caller {caller}."
                    )
    ...
    def record_response(
        ...
        files_hash = truncate_hash(
            self._calculate_files_hash(source_file, test_file), self.HASH_DISPLAY_LENGTH
        )
            ...
            try:
                with open(response_file, "r") as f:
                    ...
                    if isinstance(loaded_data, dict):
                        ...
                        cached_data.update(
                            {k: v for k, v in loaded_data.items() if k != meta_key_name}
                        )
                        self.logger.debug(
                            f"Loaded existing LLM record with {len(cached_data) - 1} entries."
                        )
        ...
        # Create entry
        prompt_hash = truncate_hash(
            hashlib.sha256(str(prompt).encode()).hexdigest(), self.HASH_DISPLAY_LENGTH
        )
        self.logger.info(
            f"🔴 Recording new LLM response for {caller_name}() (prompt hash {prompt_hash})..."
        )
    ...
    def _calculate_files_hash(self, source_file: str, test_file: str) -> str:
        ...
        if self.files_hash:
            self.logger.debug(
                f"Using cached files hash {truncate_hash(self.files_hash, self.HASH_DISPLAY_LENGTH)}."
            )
            return self.files_hash

        self.logger.debug(
            f"Calculating hash for files {source_file} and {test_file}..."
        )
        ...
        self.logger.info(
            f"Generated new files hash {truncate_hash(self.files_hash, self.HASH_DISPLAY_LENGTH)}."
        )
        return self.files_hash
    ...
    def _get_response_file_path(self, source_file: str, test_file: str) -> Path:
        ...
        test_name = os.getenv(
            "TEST_NAME", "default"
        )  # Use TEST_NAME env variable or default to "default"

        # For debug needs when running tests not in a container. May be removed in the future.
        if test_name == "default":
            test_name = (
                Path(source_file).parts[-2]
                if len(Path(source_file).parts) >= 2
                else test_name
            )

        # Get the absolute file path
        response_file_path = (
            self.base_dir / f"{test_name}_responses_{files_hash}.yml"
        ).resolve()
    ...
    def _find_closest_prompt_match(
        ...
        """
        self.logger.info(
            f"Starting fuzzy prompt matching with {len(recorded_prompts)} recorded prompts..."
        )
        ...
        for prompt_hash, prompt_data in recorded_prompts.items():
            ...
            self.logger.info(
                f"Comparing with {prompt_hash}: similarity ratio={ratio}..."
            )

            if ratio > best_ratio:
                self.logger.info(
                    f"New best match found for prompt hash {prompt_hash} with ratio {ratio}."
                )
                best_ratio = ratio
        ...
        self.logger.info(
            f"Final result: best_ratio={best_ratio}, match={'found' if result else 'not found'}"
        )
