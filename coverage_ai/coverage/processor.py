...
class CoverageProcessor(ABC):
    ...
    def process_coverage_report(self, time_of_test_command: int) -> CoverageReport:
        ...
        if coverage:
            ...
            report.total_coverage = (
                (float(total_covered) / float(total_lines)) if total_lines > 0 else 0.0
            )
        return report

    def _is_coverage_valid(self, time_of_test_command: int) -> None:
        """
    ...
    def _is_report_obsolete(self, time_of_test_command: int) -> bool:
        ...
        """
        return (
            int(round(os.path.getmtime(self.file_path) * 1000)) < time_of_test_command
        )
...
class CoberturaProcessor(CoverageProcessor):
    ...
    def _parse_coverage_data_for_class(self, cls) -> CoverageData:
        ...
        coverage_percentage = (
            (float(len(lines_covered)) / total_lines) if total_lines > 0 else 0.0
        )
        return CoverageData(
            lines_covered,
            len(lines_covered),
            ...
            with open(self.file_path, "r") as file:
                for line in file:
                    ...
                    if line.startswith("SF:"):
                        ...
                        total_lines = len(lines_covered) + len(lines_missed)
                        coverage_percentage = (
                            (float(len(lines_covered)) / total_lines)
                            if total_lines > 0
                            else 0.0
                        )
                        coverage[filename] = CoverageData(
                            lines_covered,
                            len(lines_covered),
                            lines_missed,
                            len(lines_missed),
...
class JacocoProcessor(CoverageProcessor):
    ...
    def parse_coverage_report(self) -> Dict[str, CoverageData]:
        ...
        if file_extension == "xml":
            ...
            missed, covered = self._parse_jacoco_csv(
        ...
        coverage[class_name] = CoverageData(
            covered_lines=[],
            covered=covered,
            missed_lines=[],
            missed=missed,
    ...
    def _parse_jacoco_xml(self, class_name: str) -> tuple[int, int]:
...
class DiffCoverageProcessor(CoverageProcessor):
    ...
    def parse_coverage_report(self) -> Dict[str, CoverageData]:
        ...
        coverage[self.file_path] = CoverageData(
            covered_lines=covered_lines,
            covered=len(covered_lines),
            missed_lines=violation_lines,
            missed=len(violation_lines),
            coverage=coverage_percentage,
        )
        return coverage
...
class CoverageReportFilter:
    ...
    def filter_report(
        self, report: CoverageReport, file_pattern: str
    ) -> CoverageReport:
        """
        ...
        total_lines = sum(
            len(cov.covered_lines) + len(cov.missed_lines)
            for cov in filtered_coverage.values()
        )
        total_coverage = (
            (
                sum(len(cov.covered_lines) for cov in filtered_coverage.values())
                / total_lines
            )
            if total_lines > 0
            else 0.0
        ...
        return CoverageReport(
            total_coverage=total_coverage, file_coverage=filtered_coverage
        )
...
class CoverageProcessorFactory:
    ...
    def create_processor(
        ...
        if tool_type.lower() == "diff_cover_json":
            return DiffCoverageProcessor(
                diff_coverage_report_path, report_path, src_file_path
            )
...
def process_coverage(
    ...
    processor = CoverageProcessorFactory.create_processor(
        tool_type, report_path, src_file_path, diff_coverage_report_path
    )

    # Process full report
    report = processor.process_coverage_report(
