...
Coverage Ai consists of many classes but the fundamental flow lives within the CoverageAi and UnitTestGenerator classes. The following is a sequence diagram (written in [Mermaid](https://mermaid.js.org/syntax/sequenceDiagram.html)) depicting the flow of how Coverage Ai works and interacts with a Large Language Model.

```mermaid
sequenceDiagram
    participant User
    participant CoverageAi
    ...
    CoverageAi->>CoverageAi: _duplicate_test_file()
    CoverageAi->>UnitTestGenerator: Initialize UnitTestGenerator
